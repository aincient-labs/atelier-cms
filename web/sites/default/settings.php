...
/**
 * AIncient — product settings (environment-agnostic; committed to git).
 *
 * Per-environment values and secrets live in settings.ddev.php (local, DDEV-managed)
 * and settings.local.php (any environment) — both gitignored. This block runs BEFORE
 * the DDEV include on purpose: DDEV only sets config_sync_directory `if (empty(...))`,
 * so setting it here makes DDEV honor our repo path.
 */

// Config-as-code: the sync directory lives in the repo, outside the web root.
$settings['config_sync_directory'] = '../config/sync';

// Private files, outside the web root.
$settings['file_private_path'] = '../private';

// Trusted hosts. DDEV widens this to '.*' locally; tighten per deployment in prod.
$settings['trusted_host_patterns'] = [
  '^.+\.ddev\.site$',
  '^localhost$',
];

// Modules whose enabled-state + config is never written to config/sync by `cex`
// (and never uninstalled by `cim`). Two groups:
//   - aincient_demo seeds throwaway demo content and must not ship in the
//     distribution's exported config-as-code baseline.
//   - dev tooling a contributor may enable locally (devel & friends) that must
//     never leak into config/sync. Listing modules that aren't installed is a
//     no-op, so this stays safe as a standing guard.
...
// Harden uid 1: the distribution ships with the super-user permission bypass
// DISABLED (security.enable_super_user=false in services.yml) so access is governed
// purely by roles/permissions. Local DDEV re-enables the legacy bypass (god-mode
// uid 1) for developer convenience by layering services.dev.yml on top — appended
// after the services.yml from above so the dev value wins. Gated on IS_DDEV_PROJECT,
// so the appliance/production install is never affected.
