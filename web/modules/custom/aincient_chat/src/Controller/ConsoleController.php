...
HTML;
  ...
  public function app(): HtmlResponse {
    $settings = [
      ...
      // The human-readable name of the provider an attachment's image is sent to,
      // so the composer consent line can name it ("sent to Anthropic") instead of
      // the generic "your connected AI provider". Resolved from the VISION role —
      // where the image bytes actually go via AiGateway::describeImage() (the
      // pre-describe seam, DECISIONS 0361) — which falls back to the default chat
      // provider when unbound. NULL when nothing is connected/resolvable, and the
      // client keeps the generic line. Not a code control: access on our disk is
      // moot once the bytes leave; this is a consent affordance.
      'provider' => $this->attachmentProviderName(),
      // The studio fields the chat AGENT can actually write, as namespaced keys
      // ("identity.name", "chrome.header.logo_size", …). The console marks every
      // field NOT in this set with a "set directly" hands-on badge, so a
      // human-only field never reads as one the agent can help with. Derived
      // ({@see chatReach}) from the SAME whitelists the preview tools enforce, so
      // the badge can never drift from what the agent does — when a field becomes
      // reachable its key appears here and the badge disappears (DECISIONS 0372).
      'chatReach' => $this->chatReach(),
    ];
    ...
    // Cut the exfil channel (DECISIONS 0347): an attachment can carry instructions
    // the model transcribes, so assistant output is untrusted. `img-src`/`connect-src`
    // are pinned to 'self' (+ data:/blob: for our own inline/generated bytes) so a
    // remote `<img>` or fetch in that output cannot beacon data to a third party.
    // `script-src` stays 'unsafe-inline' deliberately: assistant text is rendered as
    // react-markdown WITHOUT rehype-raw, so there is no markdown→script path, and a
    // strict script-src would break Drupal's own un-nonced inline bootstrap in this
    // chrome-less shell. The remote-image citation rendering is the paired UX control.
    $response->headers->set('Content-Security-Policy', self::CSP);
    return $response;
  }

  /**
   * The chat-pane Content-Security-Policy. See the call site for the rationale.
   */
  ...
  /**
   * The display name of the provider an attached image is sent to, or NULL.
   *
   * Resolved from the VISION role — the image bytes reach a provider only through
   * {@see \Drupal\aincient_core\AiGateway::describeImage()} (pre-describe, DECISIONS
   * 0361) — which the resolver falls back to the default chat provider for when it
   * is unbound. Returns NULL when no provider is connected or the id has no adapter,
   * so the composer keeps its generic consent line rather than naming nothing.
   */
  private function attachmentProviderName(): ?string {
    $providerId = $this->modelRoles->resolve(ModelRoles::VISION)['provider_id'] ?? '';
    if ($providerId === '') {
      return NULL;
    }
    try {
      return $this->providers->adapter($providerId)->label();
    }
  ...
  /**
   * The studio fields the chat agent can WRITE, as namespaced keys.
   *
   * The single source of truth the console's hands-on marker reads: a studio
   * field whose key is absent here is human-only and earns the "set directly"
   * badge. DERIVED — never hand-listed — from the very whitelists the preview
   * tools enforce, so the badge cannot drift from what the agent actually does:
   *  - identity prose: {@see SiteIdentity::GUIDELINE_KEYS} + `footer_note`, the
   *    keys {@see \Drupal\aincient_pages\ChromePreviewApplier} lets through; and
   *  - header/footer layout: every key of {@see ChromeRepository::REGISTRY}.
   * Logo, favicon, site info, menus and privacy have NO preview tool, so they
   * never appear here and stay marked (DECISIONS 0372).
   *
   * @return list<string>
   */
  private function chatReach(): array {
    ...
    // ChromePreviewApplier whitelists footer_note alongside the guideline keys.
    $keys[] = 'identity.footer_note';
    foreach (ChromeRepository::REGISTRY as $section => $settings) {
      foreach (array_keys($settings) as $key) {
        $keys[] = 'chrome.' . $section . '.' . $key;
      }
    }
    return $keys;
  }
